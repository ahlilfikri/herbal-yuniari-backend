...
module.exports = {
  get: async (req, res) => {
    try {
      const content = await partnerModel.find();
  ...
  getSingle: async (req, res) => {
    id = req.params.id
    try {
      const content = await partnerModel.findOne({_id:id});
    ...
    upload(req, res, async (error) => {
      if (error instanceof multer.MulterError) {
        ...
        try {
          ...
          const newPartnerModel = new partnerModel({
            title,
            image,
          });
          await newPartnerModel.save();
          response(201, newPartnerModel, 'data berhasil di tambahkan', res);
        } catch (error) {
    ...
    upload(req, res, async (error) => {
      if (error instanceof multer.MulterError) {
        ...
        try {
          ...
          const updatedPartnerModel = await partnerModel.findByIdAndUpdate(
            id,
            update,
            { new: true }
          );
          response(200, updatedPartnerModel, 'data berhasil diperbarui', res);
        } catch (error) {
          console.log(error.message)  
